...
// Derived from myfanhua/turb-gpt-free-register under the MIT license.
// See sentinel_node_runner.LICENSE in this directory.
...
function readConfig(args) {
  if (args["no-config"] === "1") return { path: null, data: {} };
...
async function main(argv = process.argv.slice(2), writeOutput = true) {
  ...
  const configuredSdkPath = pick(args["sdk"], cfg("sdk", "sdkPath"), process.env.SENTINEL_SDK_PATH);
  const defaultSdkPath = configuredSdkPath
    ? ""
    : (fs.existsSync(path.resolve(__dirname, "sdk.js"))
      ? path.resolve(__dirname, "sdk.js")
      : path.resolve(__dirname, "..", "sdk.js"));
  const sdkPath = path.resolve(pick(configuredSdkPath, defaultSdkPath));
  ...
  const includeSo = args["include-so"] === "1" || truthy(cfg("includeSo", "include_so"));
  ...
  if (includeSo) {
    ...
    soTokenText = await context.SentinelSDK.sessionObserverToken(flow);
  }
  clearTimers();
  const bundleText = JSON.stringify({ token: tokenText, soToken: soTokenText });
  if (!writeOutput) return bundleText;
  if (args.pretty || process.env.SENTINEL_PRETTY === "1") {
    process.stdout.write(`${JSON.stringify(JSON.parse(bundleText), null, 2)}\n`);
  } else {
