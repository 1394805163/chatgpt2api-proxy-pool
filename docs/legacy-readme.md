...
- 设置页新增 Free 号池死号清理，可按间隔强校验正常 Free 账号，并在生图连续失败后强验证该账号。
- Free 号池死号清理支持“标记异常并额度归零”或“直接删除”，适合一次性 Free 号池更果断地清理死号。
- 注册补号判断前可启用 Free 号池强校验，避免死号占着额度导致补号任务误判“当前额度已达标”。
- 普通文本对话链路新增 `reasoning_effort` / `thinking_effort` 透传，兼容 `low`、`medium`、`high`、`xhigh` / `extended`。
- Debug 对话页新增“思考强度”下拉和图片输入，方便直接测试文本思考强度与多模态对话请求。
...
设置页新增 `启用 Free 号池死号清理`：

- 只处理 `type=free` 的账号，不影响 Plus / Team / Pro 等账号。
- 后台 watcher 会按 `定期强校验间隔` 刷新正常 Free 账号，及时发现已经失效的号。
- 生图链路会记录连续失败次数，达到 `生图失败阈值` 后会强验证该账号。
- 注册补号判断前可启用强校验，避免死号长期占用“正常账号”和“剩余额度”统计，导致补号任务误判不需要注册。
- 清理方式支持 `标记异常并额度归零` 和 `直接删除`；一次性 Free 号池更适合直接删除。

这个功能适合 Free 号池长时间挂机场景。它的目的不是替代账号刷新，而是让“已经实际不可用但还留在正常池里的账号”更快被踢出，减少生图时才暴露死号的问题。
...
普通文本对话链路支持客户端显式传入思考强度，不会影响图片生成、图生图、PPT / PSD 文件任务逻辑。
...
| `medium` | `medium` |
| `high` | `high` |
| `xhigh` / `extended` | `extended` |

非法值会被忽略，不会报错。Debug 对话页新增“思考强度”下拉和图片选择，可直接测试 `/v1/chat/completions` 的 reasoning effort 与图片输入。
...
本仓库的默认 `docker-compose.yml` 会从当前源码本地构建镜像，不再拉取官方 `basketikun/chatgpt2api` 镜像。
...
它会自动创建 `data/` 和默认 `config.json`，然后执行构建与启动。
...
更新到 GitHub 最新版本后重新构建并热替换容器，Windows 用户可以直接双击：
...
一般不需要手动删除旧容器，Compose 会根据新的镜像和配置自动重建并替换服务。
...
请不要把真实账号数据、日志、Token、私有 `config.json` 或 `.env` 上传到公开仓库。推荐在 `.env` 中覆盖运行参数：

```env
CHATGPT2API_AUTH_KEY=12345678
CHATGPT2API_PORT=3000
STORAGE_BACKEND=json
```

如果要自定义 API Key，改 `.env` 里的 `CHATGPT2API_AUTH_KEY` 即可；没有 `.env` 时默认 key 是 `12345678`。
