...
class AccountCapabilityTests(unittest.TestCase):
    ...
    def test_refresh_accounts_never_delete_on_temporary_upstream_errors(self) -> None:
        original_value = config.data.get("auto_remove_invalid_accounts")
        config.data["auto_remove_invalid_accounts"] = True
        try:
            for error_message in (
                ...
                with self.subTest(error=error_message), tempfile.TemporaryDirectory() as tmp_dir:
                    service = AccountService(JSONStorageBackend(Path(tmp_dir) / "accounts.json"))
                    service.add_account_items([{"access_token": "temporary-token", "status": "正常"}])
                    ...
                    self.assertEqual(result["refreshed"], 0)
                    self.assertEqual(len(result["errors"]), 1)
                    self.assertIsNotNone(service.get_account("temporary-token"))
        finally:
            if original_value is None:
                config.data.pop("auto_remove_invalid_accounts", None)
            else:
