...
export function UserKeysCard() {
  ...
  const handleEdit = async () => {
    ...
    if (
      ...
      && nextImageRetentionMinutes === (item.image_retention_minutes ?? 0)
    ) {
      setEditingItem(null);
      return;
    }
                  ...
                  <div key={item.id} className="flex flex-col gap-3 rounded-xl border border-stone-200 bg-white px-4 py-4 md:flex-row md:items-center md:justify-between">
                    <div className="min-w-0 space-y-2">
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-stone-500">
                        ...
                        <span>图片保留 {item.image_retention_minutes ? `${item.image_retention_minutes} 分钟` : "沿用全局"}</span>
                      </div>
                    </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="rounded-2xl p-6">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-stone-700">图片保留时间（分钟）</label>
              <Input
                type="number"
                min="0"
                max="43200"
                ...
                className="h-11 rounded-xl border-stone-200 bg-white"
              />
              <p className="text-xs text-stone-500">填 0 表示沿用全局保留时间；例如 30 或 60 分钟。</p>
            </div>
          </div>
          <DialogFooter>
            <Button
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="h-11 rounded-xl border-stone-200 bg-white"
                />
                <p className="text-xs text-stone-500">填 0 表示沿用全局保留时间。</p>
              </div>
            </div>
          </div>
          <DialogFooter>
