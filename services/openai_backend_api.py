...
class OpenAIBackendAPI:
    ...
    def _image_model_settings(self, model: str) -> tuple[str, str]:
        ...
        elif base_model not in {"gpt-image-2", CODEX_IMAGE_MODEL}:
            return "auto", ""
    ...
    def iter_codex_image_response_events(
        ...
        request_timeout = self._image_request_timeout(1200)
        logger.info({
    ...
    def _poll_image_results(
        ...
        def _remaining() -> float:
            ...
            if deadline_ts is not None:
                remaining = min(remaining, deadline_ts - time.time())
        ...
        while _remaining() > 0:
            ...
            if not file_ids and not sediment_ids:
                ...
                if policy_msg:
                    ...
                    error = ImageContentPolicyError(policy_msg)
    ...
    def resolve_conversation_image_urls(
        ...
        if poll and conversation_id:
            ...
            try:
                ...
                if not file_ids and not sediment_ids:
                    if task_error:
                        error = ImageContentPolicyError(task_error)
                        setattr(error, "conversation_id", conversation_id or "")
    ...
    def _stream_picture_conversation(
        ...
        self.image_deadline_ts = task_deadline_ts
        self.image_cancel_event = cancel_event
        self._ensure_image_task_active()
        ...
        def close_on_deadline() -> None:
            while not stop_closer.wait(0.25):
                try:
                    self._ensure_image_task_active()
                except ImageTaskDeadlineError:
                    try:
