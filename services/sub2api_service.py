...
def _fetch_access_tokens_for_accounts(server: dict, account_ids: list[str]) -> tuple[list[str], list[dict]]:
    """Export selected accounts in one request and return tokens plus per-account errors."""
    base_url = _clean(server.get("base_url"))
    headers = _auth_headers(server)
    ...
    for account in accounts:
        ...
        if access_token:
            ...
            errors.append({"name": account_id or "unknown", "error": "missing access_token"})

    for account_id in ids:
        if account_id not in returned_ids:
            errors.append({"name": account_id, "error": "account missing from export"})

    return list(dict.fromkeys(tokens)), errors
    ...
    def _run_import(self, server_id: str, server: dict, account_ids: list[str]) -> None:
        ...
        try:
            ...
            for error in errors:
                self._append_error(
                    server_id,
                    _clean(error.get("name")) or "unknown",
                    _clean(error.get("error")) or "unknown error",
                )
